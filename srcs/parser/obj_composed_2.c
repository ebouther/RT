...
/*   Updated: 2016/05/18 15:49:29 by ascholle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_treecpy(t_nod **dest, t_nod *src)
{
	*dest = (t_nod *)malloc(sizeof(t_nod));
	...
	else
	{
		...
		(*dest)->obj = NULL;
		(*dest)->op = src->op;
		ft_treecpy(&(*dest)->l, src->l);
		ft_treecpy(&(*dest)->r, src->r);
	}
}
...
//	char	*position;
	...
	while (e->c_obj)
	{
		...
		{
			//cpy a implementer
			ft_treecpy(&nod, e->c_obj->content);
			//nod = e->c_obj->content;
