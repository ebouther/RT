...
static int	ft_set_quadra(char *quadra, t_env *e)
{
	char	*position;
	char	*radius;
	char	*equ;
	...
	if ((position = ft_get_inner(quadra, "position", NULL)) == NULL)
		ft_error_exit("Error: quadra require a position subobject.\n");
	if ((rotation = ft_get_inner(quadra, "rotation", NULL)) == NULL)
		ft_error_exit("error: camera require an angle subobject.\n");
	if ((equ = ft_get_inner(quadra, "equ", NULL)) == NULL)
		ft_error_exit("error: camera require an angle subobject.\n");
	if ((radius = ft_get_inner(quadra, "radius", NULL)) == NULL)
		ft_error_exit("Error: quadra require a radius subobject.\n");
	if ((mat = ft_get_inner(quadra, "mat", NULL)) == NULL)
...
int			ft_get_quadras(char *objects, size_t len, t_env *e)
{
	...
	pos = 0;
	while ((quadra = ft_get_inner(objects, "quadra", &pos)) != NULL)
	{
		ft_set_quadra(quadra, e);
		ft_strdel(&quadra);
		if ((int)len - pos < 0)
			break ;
		objects += pos;
